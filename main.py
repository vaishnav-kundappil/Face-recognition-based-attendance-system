...
######################################## DELETE PROFILE ############################################
def delete_profile():
    id_to_delete = txt.get()  # Get the ID of the profile to delete
    exists = os.path.isfile("StudentDetails\StudentDetails.csv")
    if exists:
        # Read the CSV file and filter out the profile to delete
        df = pd.read_csv("StudentDetails\StudentDetails.csv")
        df = df[df['ID'] != id_to_delete]
        # Save the updated CSV file without the deleted profile
        df.to_csv("StudentDetails\StudentDetails.csv", index=False)
        # Count the total registrations after deletion
        total_registrations = len(df) // 2 - 1
        message.configure(text='Total Registrations till now: ' + str(total_registrations))
        # Show a message indicating successful deletion
        mess._show(title='Profile Deleted', message='Profile with ID ' + id_to_delete + ' has been deleted.')
    else:
        mess._show(title='No Data', message='No data found to delete.')
...
deleteProfile = tk.Button(frame2, text="Delete Profile", command=delete_profile, fg="white", bg="red", width=34,
                          height=1, activebackground="white", font=('times', 15, ' bold '))
deleteProfile.place(x=30, y=400)
